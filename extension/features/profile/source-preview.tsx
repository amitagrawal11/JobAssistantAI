...
export function SourceFactVerification({ profileId, documentId }: { profileId: string; documentId: string }) {
  ...
  if (profileQuery.isError || previewQuery.isError || !profile || !preview || !page) return <section className="state-panel error"><AlertTriangle /><h2>Source comparison unavailable</h2><p>{profileQuery.error?.message ?? previewQuery.error?.message ?? 'The resume preview could not be loaded.'}</p><Button variant="secondary" onClick={() => { void profileQuery.refetch(); void previewQuery.refetch(); }}>Try again</Button></section>;
