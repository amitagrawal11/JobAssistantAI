...
export function FillView() {
  ...
  return <div className="step-stack fill-view"><div><div className="section-heading"><p className="eyebrow">Application fields</p><strong>{ready}/{plan.entries.length}</strong></div><Progress value={Math.round((ready / plan.entries.length) * 100)} label="Application field readiness" /></div>{groups.map(([title, predicate]) => { const entries = plan.entries.filter(predicate); return <section key={title}><div className="section-heading"><h2>{title}</h2><strong>{entries.length}</strong></div><div className="field-list">{entries.map((entry) => <FieldMappingRow key={entry.fieldId} entry={entry} onSelect={(selected) => void approve(entry.fieldId, selected)} onAnswer={(value) => void answer(entry.fieldId, value)} />)}</div></section>; })}<label className="overwrite-option"><input type="checkbox" disabled /> Allow overwrite of selected fields <small>Unavailable in Phase 1 simulation</small></label></div>;
}
