...
export function ChangeCard({ change, onApprove, onReject }: { change: Change; onApprove: () => void; onReject: () => void }) {
  ...
  return <article className={`change-card ${change.status}`}>
    ...
    <div className="change-actions"><Button size="sm" variant="secondary" onClick={onReject}><X size={14} /> Reject</Button><Button size="sm" disabled={unsupported} onClick={onApprove}><Check size={14} /> Accept</Button></div>
  </article>;
}
