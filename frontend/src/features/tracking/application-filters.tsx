...
export function ApplicationFilters() {
  ...
  return <div className="segmented" aria-label="Application filters">{filters.map((filter) => <Button key={filter} size="sm" variant={current === filter ? 'default' : 'secondary'} onClick={() => setFilter(filter)}>{filter[0].toUpperCase() + filter.slice(1)}</Button>)}</div>;
}
