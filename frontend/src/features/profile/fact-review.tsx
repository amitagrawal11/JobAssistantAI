  ...
  return (
    <article className={`fact-card backend-fact${selected ? ' selected' : ''}`} onClick={onSelect}>
      ...
      <div className="fact-actions">
        ...
        <Button size="sm" variant="secondary" disabled={fact.verified || pending} onClick={(event) => { event.stopPropagation(); onVerify(); }}><CheckCircle2 size={14} /> {fact.verified ? 'Verified' : 'Verify'}</Button>
      </div>
    </article>
  );
}
